...
- Accepted topics live in `data/topics/*.yaml`.
- `topic_id` must match the filename and is an internal stable key.
- `parent_id` defines the topic tree.
- `display_order` is UI ordering only; it is not a rank or importance score.
- `baselines` and `review_triggers` are direct to the topic where they are declared.
